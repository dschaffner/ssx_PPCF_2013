...
\begin{equation}
S_{p}(\tau) = \int_{-x_{max}}^{x_{max}} P(\Delta \dot{B})(\Delta \dot{B})^{p} dx = \langle \Delta \dot{B}_{\tau}(t) \rangle
\label{eq:structurefunc}
\end{equation}
with $x$ representing a histogram bin width. An exact Gaussian distribution would have a flatness equal to $3$. Note that as Equation~\ref{eq:structurefunc} indicates, the flatness can be equivalently calculated a moment of the PDF or directly though an average of the timeseries of deltas. 
